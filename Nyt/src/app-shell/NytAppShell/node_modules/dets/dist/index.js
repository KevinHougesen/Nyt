...
function includeClauses(context, clauses, newClause, tags = []) {
  ...
  clauses.push({
    ...newClause,
    types: ts2.factory.createNodeArray(types)
  });
}
...
var DeclVisitor = class {
  ...
  getInterface(node) {
    ...
    const type = checker.getTypeAtLocation(node);
    const decls = type.symbol.declarations.filter(ts3.isInterfaceDeclaration).filter((m) => !isImportedFile(m, root, availableImports));
    const clauses = [];
    const props = [];
  ...
  getExpression(node) {
    if (ts3.isArrowFunction(node)) {
      const typeNode = this.convertToTypeNodeFromNode(node);
      if (ts3.isFunctionTypeNode(typeNode)) {
        return this.getMethodSignature(node);
      }
      return this.getTypeNode(typeNode);
